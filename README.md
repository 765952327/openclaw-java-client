...
- [ ] **Spring Boot Starter** - 自动配置支持 ✅ 已完成

### Spring Boot Starter

添加 Maven 依赖后，Spring Boot 会自动配置 `OpenClawWsClient`。
...
public class MyController {
    ...
    public void test() {
        AgentResult result = wsClient.runAgent("你好");
    }
}
```
...
| 属性 | 默认值 | 描述 |
|------|--------|------|
| `ws.enabled` | true | 是否启用 WebSocket 客户端 |
| `ws.base-url` | http://127.0.0.1:18789 | Gateway 地址 |
| `ws.token` | - | Gateway 令牌 |
| `ws.require-device` | false | 是否需要设备认证 |
| `ws.auto-connect` | false | 启动时自动连接 |
| `ws.max-queue-capacity` | 500 | 请求队列最大容量 |
| `ws.default-request-timeout-ms` | 60000 | 请求超时(毫秒) |
| `ws.default-result-timeout-ms` | 300000 | 结果超时(毫秒) |
